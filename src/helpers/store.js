...
export const useStore = create(devtools(
  // persist(
    ...
    masterTrigger: 1,
    resetIndexTriggersStores: () => set({ masterTrigger: 3,
      canvasTriggers: {
        animationTriggers: 2,
        textTriggers: 2,
        sunTriggers: 1,
        ...
        if (currentTime - lastIncrementTime >= 500 && !isAnimationPlaying) {
          set((state) => {
            ...
            canvasTriggers: {
              ...state.canvasTriggers,
              animationTriggers:
                trigger === 3 || trigger === 5 || trigger === 7 || trigger === 9 || trigger === 11 ?
              ...
              sunTriggers:
                trigger === 1 ?
                (state.canvasTriggers?.sunTriggers || 0) + 1 :
                state.canvasTriggers?.sunTriggers,
              },
            domTriggers: {
              ...state.domTriggers,
              modalTriggers:
                trigger === 5 || trigger === 6 || trigger === 7 || trigger === 8 || trigger === 9 ? 
                (state.domTriggers.modalTriggers || 0) + 1 :
                state.domTriggers.modalTriggers,
              domTrigger2:
                trigger === 5 ? 
